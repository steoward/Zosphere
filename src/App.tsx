      ...
      <div className="relative w-full h-full">
        ...
        {view === 'globe' && (
          <div className="absolute top-6 left-0 right-0 px-8 flex justify-between items-start z-40 pointer-events-none">
            ...
            <div className="absolute top-6 right-8 flex items-center pointer-events-auto">
              <div className="flex items-center bg-white/20 backdrop-blur-xl border border-white/40 rounded-full p-1 shadow-[0_8px_32px_rgba(0,0,0,0.3)]">
                <button 
                  onClick={() => openWindow('settings')}
                  className="p-2.5 rounded-full hover:bg-white/30 transition-colors"
                >
                  <Settings size={20} className="text-white drop-shadow-md" />
                </button>
                <div className="w-px h-6 bg-white/40 mx-1"></div>
                <button className="p-2.5 rounded-full hover:bg-white/30 transition-colors">
            ...
            <div className="absolute top-6 left-1/2 -translate-x-1/2 pointer-events-auto">
              <div 
                ...
                className="flex items-center justify-between w-[400px] bg-white/20 backdrop-blur-xl border border-white/40 rounded-full px-6 py-3 shadow-[0_8px_32px_rgba(0,0,0,0.3)] hover:bg-white/25 transition-all cursor-text group"
              >
                <span className="text-gray-200 group-hover:text-white text-sm font-medium drop-shadow-md">ابحث عن أي شيء...</span>
                <div className="p-1.5 bg-white/20 rounded-full shadow-inner">
                  <Search size={18} className="text-white drop-shadow-md" />
                </div>
              </div>
            ...
            <div className="absolute top-6 left-8 pointer-events-auto">
              <button className="relative p-3 bg-white/20 backdrop-blur-xl border border-white/40 rounded-full hover:bg-white/30 transition-colors shadow-[0_8px_32px_rgba(0,0,0,0.3)]">
                <Inbox size={22} className="text-white drop-shadow-md" />
                <span className="absolute top-2 right-2 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-[#020617] shadow-sm"></span>
              </button>
            </div>
        ...
        {openWindows.includes('search') && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm pointer-events-auto">
            <div className="w-full max-w-4xl h-[80vh] bg-white/10 backdrop-blur-2xl border border-white/20 rounded-3xl shadow-[0_8px_32px_rgba(0,0,0,0.5)] flex flex-col overflow-hidden relative">
              {/* Header */}
              <div className="flex justify-between items-center p-4 border-b border-white/10 bg-white/5">
                <h2 className="text-lg font-semibold text-white drop-shadow-md">البحث في الشبكة</h2>
                <button 
        ...
        {openWindows.includes('settings') && (
          <Window id="settings" title="الإعدادات" defaultPosition={{ x: 50, y: 50 }} width={400} height={300}>
            <div className="flex flex-col space-y-4 text-white p-4 h-full bg-white/5 backdrop-blur-xl rounded-2xl">
